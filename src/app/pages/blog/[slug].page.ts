...
export default class BlogPostPage {
  ...
  // Track cleanup functions for event listeners and timers
  ...
  private enhanceCodeBlocks(): void {
    ...
    preElements.forEach((pre) => {
      ...
      // Copy button - using Renderer2 for security
      const copyBtn = this.renderer.createElement('button');
      this.renderer.addClass(copyBtn, 'code-copy-btn');
      this.renderer.setAttribute(copyBtn, 'aria-label', 'Copy code');
      this.setCopyButtonContent(copyBtn, false);
  ...
  /**
   * Set copy button content using Renderer2 for security (no innerHTML)
   */
  private setCopyButtonContent(
    ...
    this.renderer.setAttribute(path, 'd', isCopied ? CHECK_ICON_PATH : COPY_ICON_PATH);
